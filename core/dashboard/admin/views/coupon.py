...
class AdminCouponView(LoginRequiredMixin,AdminPermissions,ListView):
    ...
    def get_queryset(self):
        ...
        if category_id := self.request.GET.get("category_id"):
            if category_id == '1':
                queryset = queryset.annotate(used_count=Count('used_by')).exclude(max_limit_usage=F('used_count'))
            elif category_id == '2':
                queryset = queryset.annotate(used_count=Count('used_by')).filter(max_limit_usage=F('used_count'))
